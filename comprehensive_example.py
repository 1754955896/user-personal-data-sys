...
def main():
    ...
    # 格式: {"文件名": {"原字段名": "新字段名", ...}}
    file_mappings = {
        # contact.json 字段映射 - 仅重命名部分字段并新增一个字段
        "contact.json": {
            "name": "联系人姓名",
            "phoneNumber": "电话号码",
            "relation": "关系",
        ...
        # event_call.json 字段映射 - 重命名所有主要字段
        "event_call.json": {
            "event_id": "事件ID",
            "type": "事件类型",
            "phoneNumber": "电话号码",
            ...
            "status": "状态",
            "+": "处理结果"  # 为所有通话记录添加处理结果字段
        },
        
        # event_gallery.json 字段映射 - 重命名部分字段
        "event_gallery.json": {
            "event_id": "事件ID",
            "caption": "照片说明",
            "datetime": "拍摄时间",
        ...
        # event_push.json 字段映射 - 重命名部分字段并新增两个字段
        "event_push.json": {
            "event_id": "事件ID",
            "title": "通知标题",
            "content": "通知内容",
    ...
    for file_name, mappings in file_mappings.items():
        if mappings:
            rename_count = sum(1 for key in mappings if key != '+')
            new_count = sum(1 for key in mappings if key == '+')
            nested_count = len(file_nested_mappings.get(file_name, {})) if file_name in file_nested_mappings else 0
            
            print(f"- {file_name}:")
            print(f"  重命名字段数: {rename_count}")
            if new_count > 0:
                print(f"  新增字段数: {new_count}")
            if nested_count > 0:
                print(f"  处理嵌套字段数: {nested_count}")
