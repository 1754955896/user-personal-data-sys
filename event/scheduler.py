...
'''
    ...
    def event_decomposer(self,events):
        ...
        for i in split_array(events,5):
            ...
            ans += data
            #保存
            with open("event_data/event_decompose/event_decompose.json", "w", encoding="utf-8") as f:
...
2.5: 休息日会灵活安排自己的生活。
    ...
    def main_schedule_event(self,data,file):
        ...
        res = []
        for i in range(1,13):
            rest = scheduler.event_schedule(scheduler.get_events_by_month(data,2025,i),i) #逐月规划主题事件
            res+=rest
            with open(file+"event.json", "w", encoding="utf-8") as f: #保存主题事件
