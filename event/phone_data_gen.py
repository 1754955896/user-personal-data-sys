...
四、输出格式要求
输出格式严格要求,仅输出JSON格式内容，不添加任何额外文本、注释或代码块标记。只输出一个数组，无论有没有事件来源都放在该数组内。(注意推送内容中不要包含双引号,不要出现如"content":"【网易云音乐】为您推荐"长途驾驶放松音乐"歌单，陪伴您的货运旅程"这样双引号包裹双引号的情况，会导致字符串转json失败，“如果字符串中包含双引号"，请用反斜杠\转义（如\"）)。示例：
